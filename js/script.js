...
$(".show_result").on("click", function(){
	...
	if(preset_filter.highlight){
		$el.key.show();
		$el.question.empty();
		$el.question.text(" : " + questions[preset_filter.highlight].question);
		...
		for(var excel in response_options[preset_filter.highlight]){
			...
			if(isArray){
				$el.keybox.append('<div class="key"><div class="square" style="background:'+preset_filter.colors[current_color]+'"></div>'+excel+' <span data-excel="'+excel+'" class="italic">(0)</span></div>');
			}
			else{
				$el.keybox.append('<div class="key"><div class="square" style="background:'+preset_filter.colors[current_color]+'"></div>'+response_options[preset_filter.highlight][excel]+' <span data-excel="'+excel+'" class="italic">(0)</span></div>');
		...
		$.each(response_colors, function(response_i, color){
			...
			$.each(responses, function(index, response){	
				if(response[preset_filter.highlight] == response_i){
					if(!total_responses[response[preset_filter.highlight]]){
						total_responses[response[preset_filter.highlight]] = 1;
					}
					else{
						total_responses[response[preset_filter.highlight]]++;						
					}
					...
					if(!total_responses[response[preset_filter.highlight][0]]){
						total_responses[response[preset_filter.highlight][0]] = 1;
					}
					else{
						total_responses[response[preset_filter.highlight][0]]++;						
					}
					if(!total_responses[response[preset_filter.highlight][1]]){
						total_responses[response[preset_filter.highlight][1]] = 1;
					}
					else{
						total_responses[response[preset_filter.highlight][1]]++;						
					}
		...
		var sum_response = 0;
		for(var key in total_responses){
			sum_response += total_responses[key];
		}
		for(var key in total_responses){			
			$('[data-excel="'+key+'"]').text("(" + total_responses[key] + ", " + Math.ceil((total_responses[key]/sum_response)*100)+ "%)");
		}
